...
Ctrl_Steering_Power_B(miro_map,miro_map2);// 2차원 배열의 복사해준다.
	
addr=Calc_Proper_Angle_make();

addr=Ctrl_Steering_Angle_Correction_add(addr,count); // 맵의 x*Y만큼의 원형 연결 리스트를 만든다.

Ctrl_Wsd_Mounted_Cam_out(miro_map); // 미로의 맵을 출력한다.

Ctrl_Wsd_Mounted_Cam(miro_map,addr);// 시작 좌표을 찾는다.

printf("\n출발점: x = %d  y = %d  Enter을 누르면 길찾기를 시작합니다...\n",addr->prev->miro_x,addr->prev->miro_y);

getchar();  //-test

Ctrl_Proportional_value(addr,move,addr->prev->miro_x,addr->prev->miro_y);//시작 값과 저장될 링크리스트만 값을 보내어 준다.

addr=Recog_Assistance_Zone_Short(addr); //  최단 거리를 구한다.
...
Ctrl_Steering_Power_B(miro_map2,miro_map);//맵을 초기화

Ctrl_Wsd_Mounted_Cam_out(miro_map); // 미로의 맵을 출력한다.(맵을 초기상태로 복원한것을 확인);

Ctrl_Steering_Angle_completion(addr,move);//완성시킨 미로의 길이 저장된값과, 포인터 값을 넘겨주면 좀더 짧아진 길을 찍어준다.

Ctrl_Wsd_Mounted_Cam_out(miro_map); // 미로의 맵을 출력한다.

getchar();

addr=Calc_Proper_Angle(addr);// 찾은 길을 더욱 줄여준다.  첫번째--

Ctrl_Steering_Power_B(miro_map2,miro_map);//미로의 맵을 초기화

Ctrl_Wsd_Mounted_Cam_out(miro_map); //(맵을 초기상태로 복원한것을 확인);

getchar(); // 첫번째의 끝

addr=Calc_Proper_Angle(addr);// 찾은 길을 더욱 줄여준다.  두번쨰--
...
Ctrl_Steering_Power_B(miro_map2,miro_map);// 2차원 배열의 복사해준다.

Ctrl_Wsd_Mounted_Cam_out(miro_map); // 미로의 맵을 출력한다.

Ctrl_Steering_Angle_completion(addr,move);//최종 완성된 길을 찍어준다.

Ctrl_Wsd_Mounted_Cam_out(miro_map); //최종 길을 보여준다
...
//-------------------------------------------------------------------------------------
	...
	addr=Calc_Proper_Angle_make(); // tail을 저장할 주소를 할당



	addr=Ctrl_Steering_Angle_Correction_add(addr,number); //tail의 주소를 넘겨주면 그 수만큼 증가된다.
